...
| OpsAgent | Отдельный timer, 26 разрешённых имён алертов, одна ограниченная попытка на событие | Реальная модель gpt-6-astra/medium обработала синтетический тест; повторное событие не вызвало модель; штатный oneshot и повторная установка успешны |
...
## Последующее расширение OpsAgent и исправление публикации метрик

Девять новых backup-алертов подключены к автоматическому расследователю:
26 разрешённых имён, модель gpt-6-astra/medium и прежние ограничения.
Синтетическое событие mattermost/secondary прошло реальную модель; повтор
не вызвал её снова. Штатный обработчик также сохранил отчёт по реальному
BackupMetricsStale. ChatOps и sudo-политика не изменены.

Выявлено отсутствие минутного publisher: до исправления метрики обновлялись
только почасовым inventory. Добавлены corp-backup-metrics.service/timer.
Два автоматических запуска обновили timestamp публикации с интервалом 60 с
при неизменных timestamps repository probe. PrivateNetwork=yes,
install-backup --check=0; порог алерта 10 минут сохранён.
Проверка offsite выявила двойной захват platform по каноническому manifest и
устаревшей копии registry. Перечислители backup/rescan исправлены: канонический
platform имеет приоритет, registry сохраняется только как fallback.
...
отдельные алерты. Девять новых имён включены в allowlist автоматического расследователя OpsAgent
(всего 26). Добавлена ограниченная диагностика из локального кэша по сервису
и хранилищу; отчёты остаются локальными, без автоматических исправлений.
Штатная маршрутизация Alertmanager сохранена. `drills_fresh` принимает последнюю успешную запись журнала в целом и
не доказывает учение для каждого сервиса. Автоматический отчёт OpsAgent —
первичная диагностика по ps/disk/memory и ограниченному backup-кэшу,
а не доказанная причина или исправление.
Проверка установки таймера не заменяет чтение статуса попыток в SQLite.

После исправления platform захвачен ровно один раз. Snapshot a9efcd1b
восстановлен отдельным Yandex reader при закрытом доступе к writer-профилям
и age-ключу: оба unit-файла минутного publisher и стабильная тестовая история
расследования совпали по байтам и правам. SQLite integrity_check успешен,
сохранён один успешный claim. Это ограниченная проверка артефактов, а не
транзакционное восстановление рабочей истории или полное учение на VPS.
